...
/*
	TODO :
		...
		- improve visibility guesses by taking inheritance visibility into account
		- improve visibility guesses by taking context hierarchy into account
*/
...
static void substitute(QByteArray& b, const QByteArray& tpl, const QByteArray& crt)
{
...
static QList<QByteArray> split(const QByteArray& b, const char *str, char in, char out)
{
	int i = 0, last = 0;
	QList<QByteArray> l;
	const int length = qstrlen(str);
	...
	//qDebug("split \"%s\" at \"%s\" => %s", b.constData(), str, QModel::join(l, ",").constData());
...
{
	int i = 0, max = context.count();
	QTokenList l = split(unscoped, "::", '<', '>');
	
	while ( (max > 0) && (i < l.count()) )
	{
...
//	qDebug() << "max scoping between " << l << " and " << context << " is " << max;
	
	return max;
}
*/

extern QByteArray join(const QList<QByteArray>& l, QByteArray glue, int max = -1);

QCodeCompletionBackend* CppCompletion::pBackend = 0;
unsigned long CppCompletion::instances = 0;

CppCompletion::CppCompletion(QObject *p)
...
CppCompletion::CppCompletion(QCodeModel *m, QObject *p)
...
{
	--instances;
	//qDebug("CppCompletion : %i remaining", instances);
...
QCodeCompletionEngine* CppCompletion::clone()
{
	CppCompletion *e = new CppCompletion(pModel);
...
QString CppCompletion::language() const
{
	return "C++";
}

QStringList CppCompletion::extensions() const
{
	QStringList l;
...
void CppCompletion::setCodeModel(QCodeModel *m)
{
	pModel = m;
}

void CppCompletion::init()
{
	if ( pBackend )
		pBackend->init();
}
...
static bool matchBackward(	Iterator& i,
							QToken open, QToken close,
							const Iterator& beg)
{
	int nest = 1;
...
template <typename Iterator>
static bool matchForward(	Iterator& i,
							QToken open, QToken close,
							const Iterator& end)
{
	int nest = 1;
					...
					QTokenList stops = QTokenList() << ";")
{
	...
	while ( i < end )
	{
		...
		if ( *i == "<" )
		{
			...
			++i;
			--tpl_nest;
			stillType = stillType && (tpl_nest || *i == "*" || *i == "&" || *i == "::");
			continue;
		}
...
QTokenList concreteTemplates(const QToken& type)
{
	...
	int	t_beg = type.indexOf('<'),
		t_end = type.lastIndexOf('>');
	...
	for ( int i = 0; i < s.count(); ++i )
	{
		if ( s.at(i) == '*' )
		{
			...
			--i;
					...
					!(pwrd && (((i + 1) < s.count()) ? (isalnum(s.at(i + 1)) || (s.at(i) == '_')) : false))
				)
		{
			s.remove(i, 1);
			--i;
...
static void updateContext(QCodeNode *p, QList<QCodeNode*> *cxt, QList<QByteArray> *scxt)
{
	if ( !p )
		return;
...
static void updateTemplates(QCodeNode *n, const QByteArray& type, QHash<QToken, QToken>& tpl)
{
	if ( !n )
		return;
	...
	int max = qMin(abs.count(), crt.count());
	...
	#ifdef TRACE_COMPLETION
	qDebug() << "concrete templates for " << type << ": <" << abs << "> : <" << crt << ">";
	#endif
}

static void updateTemplates(const QList<QCodeNode*>& l, const QByteArray& type, QHash<QToken, QToken>& tpl)
{
	if ( l.isEmpty() )
		return;
...
QByteArray CppCompletion::functionLookup(QCodeNode *n, const QByteArray& s)
{
	...
	while ( stack.count() )
	{
		...
		if ( ctype == QCodeNode::Group || (ctype == QCodeNode::Language && cn == "C++") || (ctype == QCodeNode::Namespace && m_namespaces.contains(cn)) )
		{
			...
			if ( tpl.count() && (abs.count() != crt.count()) )
				continue;
...
QByteArray CppCompletion::functionLookup(QCodeModel *m, const QByteArray& s)
{
...
#define TRACE(pattern, ...) qDebug("%s" pattern, __indent.constData(), ##__VA_ARGS__);
#define TRACE_IF(cond, pattern, ...) if ( cond ) qDebug("%s" pattern, __indent.constData(), ##__VA_ARGS__);
#else
#define TRACE_UP
...
QCodeNode* CppCompletion::lookup(const QByteArray& t)
{
	TRACE_UP
...
QCodeNode* CppCompletion::nsAwareLookup(const QByteArray& t)
{
	TRACE_UP
	TRACE("nsAware for %s", t.constData())
	
	for ( int k = 0; k < m_namespaces.count(); ++k )
	{
		const QByteArray& ns = m_namespaces.at(k);
		QCodeNode *n = ns.count() ? lookup(ns) : 0;
...
QCodeNode* CppCompletion::lookup(QCodeNode *n, const QByteArray& t, QList<QCodeNode*> *extra)
{
	if ( !n || t.isEmpty() )
		return 0;
	
	TRACE_UP
	TRACE("looking for %s in %s [%s].", t.constData(), n->role(QCodeNode::Name).constData(), n->qualifiedName().constData())
...
QCodeNode* CppCompletion::localLookup(const QList<QCodeNode*>& l,
											QByteArray& tt,
											QByteArray& type,
											bool& ptr,
											QHash<QToken, QToken>& tpl)
{
	...
	QCodeNode *n = l.count() ? lookup(l.last(), tt, &extra) : nsAwareLookup(tt);
...
QCodeNode* CppCompletion::decrementalLookup(const QList<QCodeNode*>& l,
											QByteArray& tt,
											QByteArray& type,
											bool& ptr,
											QHash<QToken, QToken>& tpl,
...
void CppCompletion::hierarchy(QCodeNode *n, QList<QCodeNode*>& l, QHash<QToken, QToken>& tpl)
{
	if ( !n )
		return;
...
void CppCompletion::getMembers(	QTokenList::const_iterator beg,
								QTokenList::const_iterator end,
								...
								int *filter)
{
	...
	if ( op == "::" )
	{
		...
		//} else if ( n && ntype == QCodeNode::Function ) {
			...
			{
				// methods
				QHash<QToken, QToken>::const_iterator vit = variables.constFind("this");
	...
	{
		const QByteArray& ns = m_namespaces.at(k);
		QCodeNode *kn = ns.count() ? lookup(ns) : 0;
	...
	do
	{
		...
		#ifdef TRACE_COMPLETION
		qDebug("looking for %s [%s : %i]", tt.constData(), type.constData(), (int)ptr);
		#endif
		
		n = (!bFirst && bStat) ? localLookup(cxt, tt, type, ptr, tpl) : decrementalLookup(cxt, tt, type, ptr, tpl);
		...
		if ( op == "[" )
		{
			if ( ptr )
			{
				...
				{
					if ( child && (child->role(QCodeNode::Name) == "operator[]") )
					{
		...
		{
			#ifdef TRACE_COMPLETION
			qWarning("malformed completion chain : pointer cannot mixup with %s operator", op.constData());
			#endif
			break;
		}
		...
		if ( op == "(" )
		{
			...
			foreach ( QCodeNode *child, children )
			{
				if ( child && (child->type() == QCodeNode::Function) && (child->role(QCodeNode::Name) == "operator->") )
				{
					type = child->role(QCodeNode::Return);
			...
			foreach ( QCodeNode *child, children )
			{
				if ( child && (child->role(QCodeNode::Name) == sig) )
				{
					...
					//qDebug("found matching symbol {%s, %s, %c}", symbol.constData(), op.constData(), (char)nt);
					...
					if ( (nt == QCodeNode::Function) && (op == "(") )
					{
						...
						break;
					} else if ( (nt == QCodeNode::Variable) && (op != "(") && (op != "::") ) {
						++i;
						type = child->role(QCodeNode::Type);
						break;
					} else if ( op == "::" ) {
...
void CppCompletion::complete(QCodeStream *s, const QString& trig)
{
	if ( !s || !DefaultPlugin::configKey<bool>("QCodeCompletionEngine/CppCompletion/enabled", true) )
		return;
	...
	// avoid segfaults by ensuring even CTRL + Space can't trigger completion on invalid input...
	if (
			tokens.isEmpty()
		||
			(
				!maybeIdentifier(tokens.last())
			&&
				(tokens.last() != ".")
			&&
				(tokens.last() != "->")
	...
	scope_file = DefaultPlugin::configKey<bool>("QCodeCompletionEngine/CppCompletion/file", true);
	scope_local = DefaultPlugin::configKey<bool>("QCodeCompletionEngine/CppCompletion/projects", true);
	scope_system = DefaultPlugin::configKey<bool>("QCodeCompletionEngine/CppCompletion/sysheaders", true);
	
	//qDebug("completion scope : {local, global}={%i, %i}", scope_local, scope_system);
	...
	while (
				(class_it = qFind(class_it, tokens.constEnd(), QByteArray("class")))
			!=
				tokens.constEnd()
		)
	{
	...
	*/
	...
	while ( --i >= tokens.constBegin() )
	{
		...
		if ( *i == ")" )
		{
			...
			if (
					isIdentifier(*i)
				&&
					((i-1) >= tokens.constBegin())
				&&
					(
						(
								(context.count() && (*i == context.last()))
							||
								(
									(context.count() > 2)
								&&
				...
				)
			{
				...
				*/
				...
				if ( scope_skipped )
				{
					...
					// go back to were we belong in case the "remnants" are parsed into a code tree
					
					i = qFind(i, tokens.constEnd(), QByteArray("{"));
				...
				if ( ((i-2) >= tokens.constBegin()) && ( *(i-1) == "~" ) && ( *(i-2) == "::" ) )
					--i;
				...
				if ( (i-2) >= tokens.constBegin() && *(i-1) == "::" )
				{
					...
					do {
						...
						#ifdef TRACE_COMPLETION
						qDebug("\tobject context %s::%s",
								i->constData(),
								(i+2)->constData());
						#endif
						context << *i;
						
					} while ( (i-2) >= tokens.constBegin() && *(i-1) == "::" );
				...
				while ( --tmp > iter )
				{
					...
					if ( !tpl_nest && (*tmp == ",") )
					{
						if ( t_buf.count() && n_buf.count() )
						{
							#ifdef TRACE_COMPLETION
							qDebug("\tparameter : %s %s",
									t_buf.constData(),
									n_buf.constData());
							#endif
				...
				break;
			} else if ( *i == "for" || *i == "foreach" ) {
				// attempt to extract inline decl e.g : 
				// for ( int i = 0; ; )
				// foreach ( QString s, )
				
				#ifdef TRACE_COMPLETION
				qDebug("inline extraction from %s to %s", (i + 2)->constData(), tmp->constData());
				#endif
				
				flush(i + 2, tmp, variables, QTokenList() << ";" << "," << ")");
			}
	...
	/*
		At this point :
			* frames is filled with variables maps ( <QString:id, QString:type> )
			* frames are ordered, i.e calling dequeue returns the frames in enclosing
			order. Thus you can do typing exactly like a C++ compiler would do.
	...
	*/
	...
	filter = 	QCodeCompletionWidget::Public |
				QCodeCompletionWidget::Protected |
	...
	QStringList lns = DefaultPlugin::configKey<QString>("QCodeCompletionEngine/CppCompletion/ns", QString()).split('\n');
	...
	if ( scope_file )
	{
		...
		while ( accurateScopeEnd < i )
		{
			if ( *accurateScopeEnd == "{" )
			{
				matchedScope = matchForward(accurateScopeEnd, "{", "}", tokens.constEnd());
		...
		localParser.update(m_locals, 0, tokens, 0, accurateScopeSize, false, &m_namespaces);
	...
	if ( extend && (tokens.constEnd() == t_beg) && pPopup->prefix().count() )
	{
		bool ptr = false;
	...
	if ( extend && (tokens.constEnd() == t_beg) && pPopup->prefix().count() )
	{
		...
		// BUT keep the "normal" lookup to make sure local/class variables will work as expected
		
		QList<QCodeNode*> globs;
		globs
			<< pBackend->rootNodes()
			<< pModel->findRootNodes("C++");
	...
	if ( nodes.isEmpty() && tokens.count() && (trigger == "(") )
	{
	...
	if ( nodes.count() )
	{
		if ( trigger == "(" )
		{
			...
			foreach ( QCodeNode *n, nodes )
			{
				...
				{
					if ( f->type() != QCodeNode::Function || f->role(QCodeNode::Name) != fn )
						continue;
					
					QString tip = QString::fromLocal8Bit(f->role(QCodeNode::Arguments)).prepend('(').append(')');
	...
	#ifdef TRACE_COMPLETION
	qDebug("parsing + scoping + search + display : elapsed %i ms", time.elapsed());
	#endif
}

static void fetchFiles(const QDir& dir, QStringList& fl, const QStringList& exts, bool suffixless)
{
	QFileInfoList il = dir.entryInfoList(QDir::Files | QDir::Dirs | QDir::NoDotAndDotDot | QDir::Readable);
	...
	foreach ( QFileInfo info, il )
	{
		...
		if ( exts.contains(info.suffix()) || (suffixless && info.suffix().isEmpty()) )
 ...
 : pModel(0)
{
	...
	bool enabled = DefaultPlugin::configKey<bool>("QCodeCompletionEngine/CppCompletion/enabled", true);
	bool suffixless = DefaultPlugin::configKey<bool>("QCodeCompletionEngine/CppCompletion/suffixless", true);
	bool sysheaders = DefaultPlugin::configKey<bool>("QCodeCompletionEngine/CppCompletion/sysheaders", true);
	
	if ( !enabled || !sysheaders )
		return;
	
	QStringList l = DefaultPlugin::configKey<QString>("QCodeCompletionEngine/CppCompletion/pathlist", QString()).split("\n");
	...
	foreach ( QFileInfo info, d.entryInfoList(QDir::Files | QDir::Readable) )
	{
		...
		serializer.deserialize(info.absoluteFilePath(), &ok, &src, info.lastModified());
		...
		if ( ok )
		{
			...
			qDebug("loading failed : %s", qPrintable(info.absoluteFilePath()));
		}
	}
	...
	foreach ( QString d, l )
	{
		...
		g->roles = "g@" + QFileInfo(lbl).fileName().toLocal8Bit() + "@" + d.toLocal8Bit();
...
QCodeNode* QCodeCompletionBackend::findNode(const QByteArray& type) const
{
	...
	//return m_sysheaders.count() ? m_sysheaders.at(0)->findNode(type) : 0;
}

...
static void prettify(QByteArray& s)
{
	...
	for ( int i = 0; i < s.count(); ++i )
	{
		...
		char next = (i + 1) < s.count() ? s.at(i + 1) : 0;
		bool cwrd = (i + 1) < s.count() ? (isalnum(s.at(i + 1)) || (s.at(i) == '_')) : false;
		
		if ( (s.at(i) == ' ') && !(pwrd && cwrd) && (next != '=') && (prev != '*') && (prev != '=') && (prev != '&') )
		{
			s.remove(i, 1);
			--i;
		}
...
static QCppParser::KeywordId keyword(const char *s);

QCppParser::QCppParser()
 : QCodeParser()
{
...
QString QCppParser::language() const
{
	return "C++";
}

bool QCppParser::canParse(const QString& fn) const
{
...
bool maybeIdentifier(const QToken& tok)
{
	...
	if ( isId )
	{
		for ( int idx = 0; idx < tok.count(); idx++ )
		{
			char c = tok.at(idx);
...
static void match(const QTokenList& list, int& l, int l_end,
				char beg, char end, QTokenList endCond = QTokenList())
{
	int count = 0;
...
inline bool filter(int& i, int end)
{
	return i < end;
}

QCodeLexer* QCppParser::lexer(QCodeStream *s)
{
	return new QCppLexer(s);
}
...
*/
void QCppParser::update(QCodeNode *n, QCodeLexer *l, bool check)
{
	if ( !n || !l )
		return;
	...
	#ifdef _TRACE_PARSING_
	qDebug("updating context %s in %s",
			qPrintable(sContext),
	...
	if ( true )
	{
		...
		while ( nodes.count() )
		{
			...
			//qDebug("  * %s, context : %s", node->role(QCodeNode::Name).constData(), node->context().constData());
			...
			if ( node->context() == cxt )
			{
				...
				//qDebug("removed node %s (in {0x%x, 0x%x})", node->role(QCodeNode::Name).constData(), node->parent, node->model);
				...
				if ( node != n )
				{
					//qDebug("deleted node %s", node->role(QCodeNode::Name).constData());
...
/*!
	\brief Updates a 
*/
void QCppParser::update(QCodeNode *n, QCodeLexer *l,
						QTokenList& tokens,
						int id, int end,
						bool bNeedCxt,
						QTokenList *ns)
	...
	while ( id < end )
	{
		...
		switch ( k )
		{
			...
			case KEYWORD_STRUCT :
			{
				...
				while ( filter(++id, end) )
				{
					if ( tokens.at(id) == ":" || tokens.at(id) == "{" || tokens.at(id) == ";" )
						break;
				}
				...
				{
					qWarning("Syntax error : expecting \';\' before \"class\" in %s", sContext.constData());
					dump(buffer_unused);
					++id;
					break;
				}
				...
				if ( (id - last) > 1 )
				{
					...
					{
						#ifdef _TRACE_PARSING_
						qWarning("skipped %s", (tokens.at(id - 1) == ">") ? "template specialization" : "malformed class decl");
						#endif
						break;
					}
				...
				#ifdef _TRACE_PARSING_
				qDebug("%s %s", (k == KEYWORD_STRUCT) ? "struct" : "class", name.constData());
				#endif
					...
					if ( tokens.at(id) == "}" )
					{
						if ( (id + 1 >= end) || (tokens.at(id + 1) == ";") )
						{
							...
							qWarning("Syntax error in class definition : "
									"missing semi-colon at end of input in %s", sContext.constData());
						}
					} else {
						qWarning("Syntax error in class definition : "
								"missing close bracket at end of input in %s", sContext.constData());
					}
				}
			...
			case KEYWORD_ENUM :
			{
				...
				if ( tokens.at(id+1) == "{" )
				{
					...
					qWarning("syntax error : expected \'{\' after \"enum\" in %s", sContext.constData());
					buffer_unused.clear();
					++id;
					break;
				}
				...
				if ( buffer_unused.count() )
				{
					qWarning("syntax error : expecting \';\' before \"enum\" in %s", sContext.constData());
					dump(buffer_unused);
					break;
				}
				...
				en->line = line;
				en->roles = QByteArray("e@") + name + "@@" + QByteArray::number(visibility);
				...
				while ( ++first < id )
				{
					if ( tokens.at(first) == "," || tokens.at(first) == "}" )
					{
						...
						{
							if ( lastVal.count() )
								val = lastVal + " + " + QByteArray::number(++lastValCount);
							else
			...
			case KEYWORD_UNION :
			{
				...
				if ( tokens.at(id+1) == "{" )
				{
					...
					qWarning("Syntax error : expected \'{\' after \"union\" in %s", sContext.constData());
					buffer_unused.clear();
					++id;
					break;
				}
				...
				if ( buffer_unused.count() )
				{
					qWarning("Syntax error : expecting \';\' before \"union\" in %s", sContext.constData());
					dump(buffer_unused);
					++id;
					break;
				}
				...
				un->line = line;
				un->roles = QByteArray("u@") + name + "@@" + QByteArray::number(visibility);
			...
			case KEYWORD_NAMESPACE :
			{
				...
				if ( buffer_unused.count() )
				{
					qWarning("Syntax error : expecting \';\' before \"namespace\" in %s", sContext.constData());
					buffer_unused.clear();
				...
				if ( tokens.at(id) != "{" )
				{
					qDebug("Syntax error : expected \'{\' after \"namespace\" in %s", sContext.constData());
					break;
				}
			...
			case KEYWORD_TYPEDEF :
			{
				...
				if ( buffer_unused.count() )
				{
					qWarning("Syntax error : expecting \';\' before \"typedef\" in %s", sContext.constData());
					dump(buffer_unused);
					++id;
					break;
				}
			...
			case KEYWORD_FRIEND :
			{
				...
				if ( buffer_unused.count() )
				{
					qWarning("Syntax error : expecting \';\' before \"friend\" in %s", sContext.constData());
					dump(buffer_unused);
					++id;
					break;
				}
			...
			case KEYWORD_USING :
			{
				...
				if ( buffer_unused.count() )
				{
					qWarning("Syntax error : expecting \';\' before \"using\" in %s", sContext.constData());
					dump(buffer_unused);
			...
			case KEYWORD_TEMPLATE :
			{
				...
				if ( buffer_unused.count() )
				{
					qWarning("Syntax error : expecting \';\' before \"template\" in %s", sContext.constData());
					dump(buffer_unused);
			...
			case KEYWORD_EXTERN :
			{
				...
				if ( buffer_unused.count() )
				{
					qWarning("Syntax error : expecting \';\' before \"extern\" in %s", sContext.constData());
					dump(buffer_unused);
			...
			default:
			{
				if ( token.endsWith("_EXPORT") || token.contains("_EXPORT_") )
				{
						...
						if ( keyword(buffer_unused.last()) == KEYWORD_NONE )
						{
							...
							do
							{
								if ( *ba == "<" )
								{
									...
									&&
										(
											(((ba+1) < buffer_end) && *(ba+1) == "int")
										||
											(((ba+1) < buffer_end) && *(ba+1) == "long")
										||
											(((ba+1) < buffer_end) && *(ba+1) == "double")
										)
									)
								{
									cont = true;
							...
							while ( ++ba < buffer_end )
							{
								if ( *ba == "::" || *ba == "<" )
								{
									...
									do
									{
										...
										++ba;
									} while ( (ba + 1) < buffer_end && (*ba != "," || assign_nest) && *ba != ";" );
								...
								if ( tok == "," )
								{
									...
									#ifdef _TRACE_PARSING_
									qDebug("%s %s;", type.constData(), tmp.trimmed().constData());
									#endif
									
									QCodeNode *var = getNode();
									var->line = line;
									var->roles = QByteArray("v@")
												+ tmp.trimmed()
												+ "@"
												+ type
												+ "@"
					...
					if ( assign && buffer_unused.count() >= 2 )
						assign = buffer_unused.at(buffer_unused.count() - 2) != "operator";
					
					if ( token == "(" )
					{
						// make sure we don't mistake var decl fro func decl...
						first = last = id;
						match(tokens, last, end, '(', ')', QList<QByteArray>() << ";");
						
						// variable decl with ctor (most probably...)
						if ( last == end || (last + 1) == end || tokens.at(last + 1) == "," || tokens.at(last) == ";" )
							assign = true;
					...
					if ( token == ":" )
					{
						...
						if ( buffer_unused.last() == "public" )
						{
							...
							//qWarning("weird syntactic construct in %s", sContext.constData());
						}
						
						if ( visa )
						{
							#ifdef _TRACE_PARSING_
							qDebug("[visibility changed to : %s]", buffer_unused.last().constData());
							#endif
					...
					} else if ( (token == "(") && ((id+1) < end) && (tokens.at(id+1) != "*") && !assign ) {
						// parse members
						first = id + 1;
						...
						if (
								(id >= end)			// out of range
							||
								(tokens.at(last) == ";")		// out of range and/or malformed code
							||
								(					// get rid of macros
									(tokens.at(id) != ";")		// !decl
								&&
						...
						if ( idx != -1 )
						{
							...
							if ( idx )
							{
								...
								// operator bool () const;
								// leave buffer as is so that retval computation succeeds
						...
						if ( !isCtorDtor )
						{
							if ( buffer_unused.isEmpty() )
							{
								...
								#ifdef _TRACE_PARSING_
								qDebug("skipped supposed macro : %s", name.constData());
								#endif
								break;
							}
						...
						if ( (last - first) >= 1 )
						{
							...
							do
							{
								if ( bFirstParamTok && !maybeIdentifier(tokens.at(first)) && tokens.at(first) != "..." )
								{
									bParamAbort = true;
									break;
								}
								...
								if ( !tpl_nest && tokens.at(first) == "," )
								{
									...
									arg_buff += (arg_buff.isEmpty() ? tokens.at(first) : " " + tokens.at(first));
							...
							)
						{
							...
							fct->roles += QByteArray("@")
										+ QByteArray::number(fq)
										+ "@"
										;
...
static const QCppParser::KeywordId IdTable [] = {
	// keyword ID         
	/*
	...
	*/
	...
	QCppParser::KEYWORD_ENUM,
	
	//QCppParser::KEYWORD_EXPLICIT,
	
	QCppParser::KEYWORD_EXTERN,
	...
	*/
	...
	QCppParser::KEYWORD_TYPEDEF,
	
	//QCppParser::KEYWORD_TYPENAME,
	
	QCppParser::KEYWORD_UNION,
	
	//QCppParser::KEYWORD_UNSIGNED,
	
	QCppParser::KEYWORD_USING,
...
static const int ids = sizeof(IdTable) / sizeof(IdTable[0]);
...
static QCppParser::KeywordId keyword(const char *s)
{
	...
	const QLatin1String *match = qBinaryFind(KeywordTable, invalid_match, s);
	
	return (match == invalid_match) ? QCppParser::KEYWORD_NONE :

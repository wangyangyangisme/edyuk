...
QHash<QToken, QToken> QCppLexer::m_macros;

QCppLexer::QCppLexer()
 : QCodeLexer(), bTokenized(false), bKeepMacros(false)
{
	initMacros();
}

QCppLexer::QCppLexer(QCodeStream *s)
...
void QCppLexer::initMacros()
{
	if ( m_macros.count() )
		return;
	
	QTokenList l = DefaultPlugin::configKey<QString>("QCodeParser/QCppParser/macros", QString()).toLocal8Bit().split('\n');
...
bool QCppLexer::keepMacroBlocks() const
{
	return bKeepMacros;
}

void QCppLexer::setKeepMacroBlocks(bool keep)
{
	bKeepMacros = keep;
}

void QCppLexer::setInput(QCodeStream *s, LexMode m)
{
...
QToken QCppLexer::previousToken()
{
	if ( !bTokenized )
	{
		qWarning("Empty token buffer (no stream passed to lexer)");
...
int QCppLexer::lineForToken(int tokenId, int minLine) const
{
	//minLine = 0;
...
template <typename T>
static void s_flush(T& buffer, QList<T>& data, bool a, const QHash<QToken, QToken>& macros, QList<int>& suspicious)
{
	if ( buffer.count() && a )
	{
...
#define flush(buffer, data, include) s_flush(buffer, data, include | bKeepMacros, m_macros, m_suspicious)
...
static void previous(QCodeStream *s, char c)
{
	s->ungetChar(c);
}

void QCppLexer::refreshTokens(LexMode m)
{
	if ( input->contextFile().contains(QRegExp("[\\\\/]arch[\\\\/]")) )
		return;
	...
	QByteArray uglyHackForQtIncludesUnderWin;
		...
		if ( isspace(c) )
		{
			...
			while ( c && isspace(c) )
			{
				if ( c == '\n' )
				{
					...
					if ( m_jumps.count() && m_jumps.at(m_jumps.count() - 2) == tokenBuffer.count() )
		...
		switch ( c )
		{
			...
			case '.' :
				...
				if ( seq == 3 )
				{
					...
					do { previous(input, tmp); } while ( --seq );
				}
					...
					if ( first && utmp.isSpace()  )
					{
						...
						if ( buffer == "#ifdef" )
						{
							...
							if ( branch.count() > 1 )
								...
								qWarning("too much #endif directives in %s", qPrintable(input->contextFile()));
						} else if ( buffer == "#include" ) {
							
							if ( tokenBuffer.isEmpty() && uglyHackForQtIncludesUnderWin.isEmpty() )
							{
								while ( next(input, tmp, utmp) )
									if ( !utmp.isSpace() )
										break;
								...
								if ( tmp == '\"' )
								{
									while ( next(input, tmp, utmp) )
									{
										...
										uglyHackForQtIncludesUnderWin += utmp;
									}
								}
								
							} else {
								uglyHackForQtIncludesUnderWin = "$";
							}
	...
	if ( branch.count() > 1 )
		qWarning("Unterminated conditionnals in %s", qPrintable(input->contextFile()));
	...
	while ( m_suspicious.count()  )
	{
		...
		QHash<QToken, QToken>::const_iterator it = macros.constFind(tokenBuffer.at(pos));
		...
		if ( it != macros.constEnd() )
		{
			if ( it->count() )
			{
				...
				for ( int i = 0; i < m_jumps.count(); i += 2 )
				{
					...
					//qDebug("gap (%i, %i) becomes (%i, %i)", m_jumps[i], m_jumps[i + 1], m_jumps[i] + prev, m_jumps[i + 1]);
					m_jumps[i] += prev;
				}
			}
		}
	...
	for ( int i = m_jumps.count() - 2; i > 0; i -= 2 )
	{
		...
		{
			//qDebug("merged gap (%i, %i) and (%i, %i)", m_jumps[i], m_jumps[i + 1], m_jumps[j], m_jumps[j + 1]);
			m_jumps.removeAt(i);
	...
	*/
	...
	if (
			tokenBuffer.isEmpty()
		&&
			uglyHackForQtIncludesUnderWin.count()
		&&
			(uglyHackForQtIncludesUnderWin != "$")
		)
	{
		/*
			Special handling for redirecting headers, i.e. those containing
			a single #include directive, with relative path and no other
			tokens (implemented to handle Qt headers under Windows...)
		*/
		
		// tryFetchOtherContext() may change contextFile()...
		QString oldfn = input->contextFile();
		
		if ( input->tryFetchOtherContext(uglyHackForQtIncludesUnderWin) )
		{
			qWarning("Redirection from %s to %s...",
					qPrintable(oldfn),
					qPrintable(input->contextFile()));
			
			refreshTokens(m);
		} else {
			qWarning("Redirection from %s to %s failed [%s]",
					qPrintable(oldfn),
					uglyHackForQtIncludesUnderWin.constData(),
					qPrintable(input->contextFile()));
		}

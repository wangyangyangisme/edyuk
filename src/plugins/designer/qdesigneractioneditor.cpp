...
QDesignerActionEditor::QDesignerActionEditor(QDesignerPerspective *p)
 : QDockWidget(0), pParent(p), pForm(0)
{
	setWindowTitle(tr("Actions"));
	setObjectName("x-designer/actioneditor");
	
	pInterface = QDesignerComponents::createActionEditor(p->handler(), this);
	...
	p->handler()->setActionEditor(pInterface);
}
...
void QDesignerActionEditor::retranslate()
{
	setWindowTitle(tr("Actions"));
}

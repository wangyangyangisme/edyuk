...
/*!
	\brief ctor
*/
QCodeLoader::QCodeLoader(QObject *p)
 : QThread(p)
{
...
*/
void QCodeLoader::removeParser(QCodeParser *p)
{
...
void QCodeLoader::update(QCodeNode *group, const QString& file)
{
	...
	//qDebug("update(%s, %s)", group->role(QCodeNode::Name).constData(), qPrintable(file));
...
/*!
	...
	\param group Name of the toplevel group which will get created on success
	\param files Files to load
	\param dest Destination model
*/
bool QCodeLoader::load(const QString& group, QStringList files, QCodeModel *dest)
{
	//qDebug("load(%s, (%s), %p)", qPrintable(group), qPrintable(files.join(", ")), dest);
	bool done = false;
	
	QCodeNode *g = new QCodeNode;
	g->roles = QByteArray("g@") + QFileInfo(group).fileName().toLocal8Bit() + "@" + group.toLocal8Bit();

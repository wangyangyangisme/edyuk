...
QCodeView::QCodeView(QCodeModel *m, QWidget *p)
 : QTreeView(p), m_model(0)
{
	setAutoScroll(true);
	setHorizontalScrollBarPolicy(Qt::ScrollBarAsNeeded);
...
{
	static QStringList exts = QStringList()
		<< "cpp"
		<< "cxx"
		<< "c"
		<< "cc";
	...
	{
		// open the source file instead (after checking whether it lies in header...)
		QFileInfo inf(cxt);
		
		foreach ( const QString& ext, exts )
		{
			QString scxt = cxt;
			scxt.chop(inf.suffix().count());
			scxt += ext;
		...
		emit actionRequested("open", QStringList() << cxt << "-l" << QString::number(n->line));
		return;
	}
	...
	//qDebug("%s [%s] {%s}", qPrintable(qn), qPrintable(n->data(Qt::DisplayRole).toString()), n->role(QCodeNode::NodeType).constData());
	
	//qn.remove(0, qn.indexOf("/") + 1);
	
	ptp = qn;
	rxp = QRegExp::escape(qn);
	
	rxp.replace(QRegExp("\\s*(::|\\\\[()\\[\\]{}|*$+.?^]|[,&<>])\\s*"), "\\s*\\1\\s*");
	rxp.replace(" ", "\\s+");
	
	i = rxp.indexOf("(");
	
	QString tmp = rxp.mid(i);
	tmp.replace(QRegExp("(\\\\s[+*])[\\w_]+\\\\s\\*(,|\\\\\\)\\\\s\\*$)"), "\\1[\\w_]*\\s*\\2");
	rxp = rxp.left(i) + tmp;
	
	ptp.replace(QRegExp(" (::|[()<>]) "), "\\1");
	i = ptp.indexOf("(");
	
	if ( i != -1 )
		ptp = ptp.left(i);
	
	emit actionRequested("open", QStringList() << cxt << "-s" << ptp << "-rx" << rxp);
}

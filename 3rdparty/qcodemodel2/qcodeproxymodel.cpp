...
QCodeProxyModel::QCodeProxyModel(QObject *p)
...
bool QCodeProxyModel::lessThan(const QModelIndex& left, const QModelIndex& right) const
{
	int lt = priority.indexOf(sourceModel()->data(left, QCodeModel::TypeRole).toInt()),
		rt = priority.indexOf(sourceModel()->data(right, QCodeModel::TypeRole).toInt());
	...
	*/
	...
	//return !((lt == rt) ? (QString::localeAwareCompare(ld, rd) < 0) : (lt < rt));

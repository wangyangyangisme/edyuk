...
*/
QCodeNodePool* QCodeParser::nodePool() const
{
	return m_pool;
}
...
*/
void QCodeParser::setNodePool(QCodeNodePool *p)
{
	m_pool = p;
}
...
void QCodeParser::update(QCodeNode *n, const QString& file)
{
	if ( !QFile::exists(file) || !QFileInfo(file).isReadable() )
	{
		...
		while ( l.count() )
		{
			...
			if ( tn->context() == file )
			{
				...
				while ( p && p->children.isEmpty() && (p->type() != QCodeNode::Group) )
				{
					tn = p;
					p = p->parent;
	...
	//qDebug("updating %s in %s", qPrintable(file), n->role(QCodeNode::Name).constData());

...
static QRegExp header_matcher("^\\{text/x-tag,([0-9a-fA-F]+),([^}]+)\\}\n$");
...
QCodeModel* QCodeSerializer::targetModel() const
{
	return m_model;
}

void QCodeSerializer::setTargetModel(QCodeModel *m)
{
	m_model = m;
}

QCodeNodePool* QCodeSerializer::nodePool() const
{
	return m_pool;
}

void QCodeSerializer::setNodePool(QCodeNodePool *p)
{
	m_pool = p;
}
...
void QCodeSerializer::serialize(QCodeNode *n, QTextStream& out, int indent)
{
	...
	if ( indent > 0 )
		...
		out << header_pattern
				.arg(QString::number(_version))
				.arg(QString::fromLocal8Bit(n->role(QCodeNode::Context))) << endl;
...
void QCodeSerializer::deserialize(const QString& file, bool *ok, QString *source, const QDateTime& t)
{
	QFile f(file);
...
void QCodeSerializer::deserialize(QIODevice *device, bool *ok, QString *source, const QDateTime& t)
{
	if ( !device )
		return;
	...
	if ( !header_matcher.exactMatch(line) )
	{
	...
	int version = header_matcher.cap(1).toInt(0, 0);
	
	if ( (version < _version_min) || (version > _version_max) )
	{
		qWarning("Version mismatch : %i !E [%i, %i]", version, _version_min, _version_max);
	...
	while ( !device->atEnd() )
	{
		...
		if ( spaces > indent )
		{
			...
			//if ( parent )
			//	qDebug("down a level : parent = %s", parent->role(QCodeNode::Name).constData());
		} else if ( spaces < indent ) {
			
			// up one (or more) level
			for ( int i = 0; (i < (indent - spaces)) && parent && parent->parent; ++i )
				parent = parent ? parent->parent : 0;
			
			//if ( parent )
			//	qDebug("up one level : parent = %s", parent->role(QCodeNode::Name).constData());
		} else {

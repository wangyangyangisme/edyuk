...
int QCodeNodePool::count() const
{
	return m_last;
}

int QCodeNodePool::bufferSize() const
{
	return m_size;
}

int QCodeNodePool::occupied() const
{
	return (100 * m_last) / m_size;
}
...
void QCodeNodePool::free(QCodeNode *n)
{
	if ( (n < m_nodes) || (n >= (m_nodes + m_size)) )
	{
		...
		if ( (void*)(n + 1) == (void*)(m_nodes + m_last) )
		{
			...
			while ( !n.m_pool && !n.parent && !n.model && (m_last > 0) && (m_free > 0) )
			{
				--m_free;
				--m_last;
